...
var srv = function() {
  ...
  this.connections = [];
	...
	process.on('SIGINT', () => {
		...
		db.remove('users', this.connections.map(socket => socket.user.id));
    ...
    .on('data', function(chunk) {
			...
			if (op === 1) { // text
	          ...
	          self.emit('log',  logString);
						...
						db.publish(event, payload, socket.user.id);
					}
				} catch(err) {
					self.emit('log', err.toString(), decode(chunk));
				}
    ...
    .on('end', function() {
      if (socket.user && socket.user.id) {
        ...
        self.emit('log', 'unknown connection ended');
			...
			// since this is the client emitting end (sending FIN) - server has to end as well
			socket.end();
    })
