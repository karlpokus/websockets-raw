  ...
  socket
    ...
    .on('data', function(chunk) {
      try {
        ...
        if (data.event === 'connection') {
          ...
          var logString = `${socket.id} connected. ${self.connections.length} connections`
    ...
    .on('end', function() {
      if (socket.id) {
        ...
        self.emit('log', `${socket.id} disconnected. ${self.connections.length} connections`);
      } else {
        self.emit('log', 'unknown connection ended');
      }
    })
    .on('close', function(hadErr){
      self.emit('log', `${socket.id} closed. transmission error ${hadErr}`);
    })
    .on('timeout', function(){
